...
@section('content')
    <section id="main-content">
        <section class="wrapper">
            <br>
            <h3 style="font-size: 18px"><i class="fa fa-angle-right"></i> All COMMENTS</h3>
            <div class="row mt">
                <div class="col-md-12">
                        ...
                        <div class="adv-table">
                            <table class="table table-striped table-advance table-hover" id="hidden-table-info">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($comment->image)
                                            <img style="height: 30px;" src="{{ asset('storage/comment/'.$comment->image) }}"/>
                                            @else
                                    ...
                                    <td>
                                        {{ Str::limit($comment->comment,300) }}
                                    </td>

                                    <td>

                                        @if ($comment->approve == 1)
                                            <form action="{{ route('admin.comment.approved',$comment->id) }}" method="post">
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-xs">Unapprove</button>
                                            </form>
                                        @else
                                            <form action="{{ route('admin.comment.unapproved',$comment->id) }}" method="post">
                                                @csrf
                                    ...
                                    <td>
                                        <button class="btn btn-danger btn-xs" onclick="deleteComment({{ $comment->id }})"><i class="fa fa-trash-o "></i></button>
